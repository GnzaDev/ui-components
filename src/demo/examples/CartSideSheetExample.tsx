...
export function CartSideSheetExample() {
  ...
  const [scenario, setScenario] = useState<"cart" | "activity">("cart");
    ...
    <div className="flex flex-col justify-between rounded-2xl border border-zinc-200 bg-white p-6 shadow-xs dark:border-zinc-800 dark:bg-zinc-900 w-full space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-100 pb-4 dark:border-zinc-800">
          <div>
            <h3 className="text-sm font-semibold text-zinc-900 dark:text-white">
              SideSheet (Drawer Overlay)
            </h3>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
              Spring-anchored sliding sheet with backdrop blur and escape dismissal.
            </p>
          </div>

          {/* Side Selector (Left vs Right) */}
      ...
      <div className="pt-2">
        <button
          ...
          className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-zinc-900 px-5 text-xs font-medium text-white hover:bg-zinc-800 dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-100 cursor-pointer transition-colors shadow-xs"
        >
          <ShoppingCart size={14} />
          <span>Open {side === "left" ? "Left" : "Right"} {scenario === "cart" ? "Cart" : "Activity"} Sheet</span>
        </button>
      </div>
      ...
      <SideSheet
        ...
        title={scenario === "cart" ? `Shopping Cart (${items.length} items)` : "Deployment Activity Stream"}
        side={side}
        maxWidth="max-w-xl"
      >
        {scenario === "activity" ? (
          <div className="space-y-4 py-2">
            {[
              { time: "2 min ago", title: "Release v1.4.2 Deployed", detail: "Edge cluster us-east-1 updated successfully.", type: "success" },
              { time: "15 min ago", title: "DB Schema Migration", detail: "Added index on users.auth_provider.", type: "info" },
              { time: "1 hour ago", title: "Traffic Surge Alert", detail: "Incoming requests reached 12,400 req/s.", type: "warning" },
              { time: "3 hours ago", title: "Nightly Security Scan", detail: "Zero critical CVE vulnerabilities detected.", type: "success" },
            ].map((event, idx) => (
              <div key={idx} className="rounded-xl border border-zinc-200/80 p-3.5 dark:border-zinc-800 bg-zinc-50/40 dark:bg-zinc-800/40 space-y-1">
