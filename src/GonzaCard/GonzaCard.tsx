...
export interface GonzaCardProps {
  id: string;
  isOpen: boolean;
  onClose: () => void;
  onOpen: () => void;
...
export function GonzaCard({
  ...
  const cardLayoutId = `gonza-card-container-${id}`;
  const imageLayoutId = `gonza-card-image-${id}`;
  const titleLayoutId = `gonza-card-title-${id}`;
